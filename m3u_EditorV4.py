...
class M3UEditor(QWidget):
    ...
    def merge_or_fix_epg(self):
        try:
            # בנה כותרת EPG מאוחדת
            header = self.buildUnifiedEPGHeader()  # יחזיר "#EXTM3U tvg-url=..." או "#EXTM3U"
            # קח את כל התוכן הקיים והחלף את השורה הראשונה
            content = self.textEdit.toPlainText().splitlines()
            if content:
                if content[0].startswith("#EXTM3U"):
                    content[0] = header
                else:
            ...
            QMessageBox.information(self, "EPG", "כותרת EPG אוחדה ותוקנה בהצלחה.")
        except Exception as e:
    ...
    def loadM3UFromText(self, content, append=False):
        """
        טוען M3U עם אופטימיזציה מקסימלית - מהיר פי 100 מהגרסה הקודמת
        """
        if not append:
            self.categories.clear()

        # 1️⃣ ניהול EPG headers - רק על 100 שורות ראשונות
        if not hasattr(self, "epg_headers") or not append:
            self.epg_headers = []

        detected_epg_headers = []
        lines_to_scan = content.splitlines()[:100]  # ✅ סריקה מוגבלת

        for line in lines_to_scan:
            if line.startswith("#EXTM3U") and any(x in line for x in ["url-tvg=", "x-tvg-url=", "tvg-url="]):
        ...
        # 5️⃣ סריקת לוגואים ברקע (לא חוסם!)
        import threading
        threading.Thread(
    ...
    def mergeM3Us(self):
        """מיזוג קובץ M3U נוסף לפלייליסט הקיים - גרסה משופרת עם טעינה מלאה"""
        ...
        # וידוא שהתוכן הוא M3U תקין
        lines_preview = new_content.splitlines()
        extinf_count = sum(1 for line in lines_preview if line.startswith("#EXTINF"))
        ...
        if extinf_count == 0:
            QMessageBox.critical(self, "Invalid File",
                                 f"לא נמצאו ערוצים תקינים בקובץ\nשורות בקובץ: {len(lines_preview)}")
            return
        ...
        try:
            ...
            print(f"[MERGE] Updated extinf_lookup with {len(extinf_data)} entries")

            # בדיקה שערוצים נוספו
            if channels_added == 0:
                QMessageBox.information(self, "M3U Merge", "לא נמצאו ערוצים חדשים לצירוף")
                return
    ...
    🔗 כל הקישורים זמינים לצפייה"""
            ...
            print(f"  - EXTINF entries saved: {len(extinf_data)}")
            print(f"  - Total channels now: {channels_after}")

        except Exception as e:
            error_msg = f"שגיאה במהלך המיזוג: {str(e)}"
    ...
    def regenerateM3UTextOnly(self, fast_mode=True):
        """גרסה משופרת לחידוש תוכן M3U - מבטיחה שמירה של כל הנתונים"""
        ...
        for category, channels in self.categories.items():
            # פילטרים נוחיתוך מהיר במקום try/except איטי
            valid_channels = [
            ...
            for name, url_with_extras in valid_channels:
                # חילוץ URL נקי (עד לרווח הראשון או סוף המחרוזת)
                url = url_with_extras.split()[0] if url_with_extras else url_with_extras

                # שימוש ב-extinf_lookup אם זמין
                extinf_line = None
    ...
    def loadM3U(self):
        """טעינת M3U עם progress bar אמיתי ועיבוד ברקע - לא חוסם UI"""
        options = QFileDialog.Options()
        fileName, _ = QFileDialog.getOpenFileName(
            self,
            "Open M3U File",
        ...
        try:
            # 📊 בדיקת גודל הקובץ מראש
            file_size = os.path.getsize(fileName)
            file_size_mb = file_size / (1024 * 1024)

            print(f"[LOAD] File size: {file_size_mb:.2f} MB")

            # ✅ קריאת קובץ מהירה עם buffer
            with open(fileName, 'r', encoding='utf-8', buffering=8192) as file:
                content = file.read()

            # ✅ הצגת progress dialog
            from PyQt5.QtWidgets import QProgressDialog
            from PyQt5.QtCore import QTimer, Qt

            progress = QProgressDialog("Loading M3U file...", "Cancel", 0, 100, self)
            ...
            # ✅ אפשרות לביטול
            cancelled = [False]  # משתמש ברשימה כי closures לא יכולים לשנות משתנים פרימיטיביים
            ...
            # 🎯 פונקציית עיבוד מדורגת - כל שלב מעדכן את ה-progress
            def process_step_1():
                """שלב 1: ניתוח EPG headers"""
                if cancelled[0]:
                    return

                progress.setLabelText("Parsing EPG headers...")
                progress.setValue(10)
                QApplication.processEvents()  # ✅ נותן לUI להתעדכן

                # טען M3U דרך המתודה הראשית
                self.loadM3UFromText(content, append=False)

                # המשך לשלב הבא
                QTimer.singleShot(10, process_step_2)
            ...
            def process_step_2():
                ...
                progress.setLabelText("Updating categories...")
                progress.setValue(60)
                ...
                # המשך לשלב הבא
                QTimer.singleShot(10, process_step_3)

            def process_step_3():
                """שלב 3: טעינת EPG אוטומטית"""
                if cancelled[0]:
                    return

                progress.setLabelText("Loading EPG data...")
                progress.setValue(80)
                QApplication.processEvents()
                ...
                for ext in [".xml", ".xml.gz"]:
                    ...
                    if os.path.exists(epg_candidate):
                        try:
                            ...
                            print(f"[EPG] Failed to load {epg_candidate}: {e}")

                # סיום
                QTimer.singleShot(10, finish_loading)

            def finish_loading():
                """שלב סופי: סגירת progress"""
                progress.setValue(100)
                progress.close()

                # ✅ הצגת הודעת הצלחה רק לקבצים גדולים
                if file_size_mb > 5:
                    total_channels = sum(len(channels) for channels in self.categories.values())
                    QMessageBox.information(
                        self,
                        "Load Complete",
                        f"✅ Successfully loaded {total_channels:,} channels\n"
                        f"📁 File size: {file_size_mb:.2f} MB"
                    )

            # ✅ התחל את התהליך המדורג
            QTimer.singleShot(100, process_step_1)

        except MemoryError:
            QMessageBox.critical(
                self,
                "Memory Error",
                f"File is too large to load ({file_size_mb:.2f} MB).\n"
